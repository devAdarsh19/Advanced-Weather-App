...
const Forecast = () => {
  ...
  return (
    <div className="forecast-weather">
      ...
      {forecastData && (
        <div>
          <h2><strong>Location : { forecastData.location_name }</strong></h2>
          <h3>Local Time : { forecastData.localtime }</h3>
          <h3>Condition : { forecastData.condition }</h3>
          <h3>Temperature: { forecastData.temp_c }℃ / { forecastData.temp_f }℉</h3>
          {forecastData.forecasts.map((days, index) => (
            <div key={index}>
              <br />
              <h3><strong>Date: { days.date }</strong></h3>
              <h4>Condition: { days.condition }</h4>
              {/* <h4>Avg Temperature: { days.temp_c }℃ / { days.temp_f }℉</h4> */}
              <h4>Max Temperature: { days.maxtemp_c }℃ / { days.maxtemp_f }℉</h4>
              <h4>Min Temperature: { days.mintemp_c }℃ / { days.mintemp_f }℉</h4>
              {days.hours.map((hour, hourIndex) => (
                <div key={hourIndex} style={{marginLeft: "20px"}}>
                  <br />
                  <h3><strong>Date & Time: { hour.time }</strong></h3>
                  <h4>Condition: { hour.condition }</h4>
                  <h4>Temperature: { hour.temp_c }℃ / { hour.temp_f }℉</h4>
                </div>
              ))}
            </div>
          ))}
