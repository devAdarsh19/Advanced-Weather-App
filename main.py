...
async def home_page_weather_logic():
    ...
    for city in top_cities_india:
        try:
            ...
            data = response.json()
            weather_top_cities_india.append({
                "region": data["region"],
    ...
    endpoint_response["home_popular"] = weather_top_cities_india
                
    for city in top_cities_abroad:
        try:
            response = await home_page_weather_client.get(f"http://localhost:8000/api/weather", params={"location_q": city})
            data = response.json()
            weather_top_cities_abroad.append({
                "region": data["region"],
                "temperature_c": data["temp_c"],
                "temperature_f": data["temp_f"],
                "condition": data["condition"],
    ...
    endpoint_response["other_popular"] = weather_top_cities_abroad
        
    return json.dumps(endpoint_response)
...
async def home_page_weather():
    ...
    return home_page_weather_logic()
